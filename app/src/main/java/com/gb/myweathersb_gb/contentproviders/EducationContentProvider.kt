package com.gb.myweathersb_gb.contentproviders
...
import com.gb.myweathersb_gb.utils.ID
import com.gb.myweathersb_gb.utils.NAME
import com.gb.myweathersb_gb.utils.TEMPERATURE
...
private const val ENTITY_PATH =
    "weather_entity_table" // Часть пути (будем определять путь до weather_entity_table)

class EducationContentProvider : ContentProvider() {

    private var authorities: String? = null // Адрес URI
    private lateinit var uriMatcher: UriMatcher // Помогает определить ти адреса URI
    ...
    override fun query(
        ...
        responseCursor.setNotificationUri(
            context!!.contentResolver,
            contentUri
        ) // на данном этапе в этой строке мало смысла
    ...
    override fun update(uri: Uri, values: ContentValues?, selection: String?, selectionArgs: Array<out String>?): Int {
        weatherDAO.updatetRoom(mapperRawToWeatherEntity(values))
        context!!.contentResolver.notifyChange(contentUri, null)
        return 1
    }

    //Пеерводим ContentValues в HistoryEntity
